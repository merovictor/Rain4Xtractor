...
ui <- fluidPage(
  titlePanel("Rainfall Data Extractor using CHIRPS"),
  
  fluidRow(
    column(
      width = 3, 
      # Sidebar panel for selecting date range and getting data
      wellPanel(
    ...
    column(
      width = 9,
      # Main panel to display map and visualizations
...
server <- function(input, output, session) {
  ...
  rainfall_data <- eventReactive(input$getData, {
    ...
    tryCatch({
      rainfall <- get_chirps(object = lonlat,
                             dates = c(as.character(input$startDate), 
